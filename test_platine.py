...
from board_constants import BOARD_MODBUS_PARAMETERS, BOARD_MB_REGISTERS, BOARD_REG_NAME_TO_ADDRESS, BOARD_REG_ADRESS_TO_NAME
...
mb_reg_end = BOARD_REG_NAME_TO_ADDRESS["Current_low_S2"]
...
while True:
    ...
    if not read_reg_values.isError():
        ...
        for address in range(mb_reg_start, mb_reg_end + 1):
            reg_name = BOARD_REG_ADRESS_TO_NAME[address]
            reg_fac = BOARD_MB_REGISTERS[address].get("factor")
            reg_value = int16(uint16(values[address - mb_reg_start]))
            reg_value = reg_value * reg_fac # type: ignore
            print(f"Register {reg_name} (Addr {address})(Factor {reg_fac}): {reg_value:.4f}")
        time.sleep(2)
    # """
    ...
    #     for address in range(mb_reg_start, mb_reg_end + 1):
    #         reg_name = BOARD_REG_ADRESS_TO_NAME[address]
    #         reg_mult = BOARD_MB_REGISTERS[address].get("multiplicator")
    ...
    # read=client.read_holding_registers(address = BOARD_REG_NAME_TO_ADDRESS["Current_high_S1"], count = 5, device_id=unidAddr)
    # if not read.isError():
    #     s = ''
    #     for r in read.registers:
